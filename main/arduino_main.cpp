...
void setup() {
    ...
    trailer = new pushed_follower(L_WHEELBASE,L_REAR_TO_HITCH,L_HITCH_TO_FOLLOWER_AXLE, deg2rad(15),1.5);
...
void loop() {
    ...
    if (iTimeSend <= timeNow){
        ...
        if(get_trailer_control()){ // trailer beta control
            if(sign(throttle) == -1)
                //DISPLAY_LOG("%f\n",rad2deg(
                    tmpSteering = trailer->calc_alpha(trailer_angle, trailer->calc_beta_const(des_steering) * 0.5f);
                //));
        }
